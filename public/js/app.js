...
function RR(){var c=$('MS'),e=$('EM');if(!c)return;
  ...
  var h='';for(var i=0;i<M.length;i++){var m=M[i],rw='<span class="rw" onclick="BW('+i+')">↩</span>',rw2= m.role!=='user'?'<span class="rw" onclick="RW('+i+')">✏</span>':'';
    ...
    else{var _sn=n,_sa=a,_tx=m.content;var _ms=_tx.match(/\u3010([^\u3011]+)\u3011/g);if(_ms&&_s.length){for(var _mi2=0;_mi2<_ms.length;_mi2++){var _mn2=_ms[_mi2].replace(/\u3010|\u3011/g,"");if(_mn2==="\u65c1\u767d")continue;for(var _msi=0;_msi<_s.length;_msi++){if(_s[_msi].name===_mn2){_sn=_mn2;_sa=_s[_msi].avatar||a;break}}if(_sa!==a)break}}h+='<div class="msg-row a-row"><div class="aav">'+(avt(_sa,30))+'</div><div class="msg a"><div class="sn">'+E(_sn)+'</div><div>'+H(_tx).replace(/【([^】]+)】/g,'<span style="color:#ffd700;font-weight:700">[$1]</span>')+'</div></div>'+rw+rw2+'</div>'}}c.innerHTML=h;c.scrollTop=c.scrollHeight}
