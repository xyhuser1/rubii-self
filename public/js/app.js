...
function RR(){var c=$('MS'),e=$('EM');if(!c)return;
  ...
  var h='';for(var i=0;i<M.length;i++){var m=M[i],rw='<span class="rw" onclick="BW('+i+')">↩</span>',rw2= m.role!=='user'?'<span class="rw" onclick="RW('+i+')">✏</span>':'';
    ...
    else{var _sn=n,_sa=a,_tx=m.content;var _mt=_tx.match(/【([^】]+)】/);if(_mt&&_s.length){var _mn=_mt[1];for(var _mi=0;_mi<_s.length;_mi++){if(_s[_mi].name===_mn){_sn=_mn;_sa=_s[_mi].avatar||a;break}}}_tx=_tx.replace(/【([^】]+)】/g,'<span style="color:#ffd700;font-weight:700">[$1]</span>');h+='<div class="msg-row a-row"><div class="aav">'+(_sa.indexOf('http')===0||_sa.indexOf('/uploads/')===0?'<img src="'+_sa+'" style="width:28px;height:28px;border-radius:50%;object-fit:cover">':avt(_sa,30))+'</div><div class="msg a"><div class="sn">'+E(_sn)+'</div><div>'+H(_tx)+'</div></div>'+rw+rw2+'</div>'}}c.innerHTML=h;c.scrollTop=c.scrollHeight}
