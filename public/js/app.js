...
function avt(a,s){if(a&&(a.indexOf('http')===0||a.indexOf('/uploads/')===0)){if(a.indexOf('http')===0)return'<img src="/api/proxy-image?url='+encodeURIComponent(a)+'" style="width:'+s+'px;height:'+s+'px;border-radius:50%;object-fit:cover">';return'<img src="'+a+'" style="width:'+s+'px;height:'+s+'px;border-radius:50%;object-fit:cover">'}return a||'🤖'}
...
function RR(){var c=$('MS'),e=$('EM');if(!c)return;
  ...
  var h='';for(var i=0;i<M.length;i++){var m=M[i],rw='<span class="rw" onclick="BW('+i+')">↩</span>',rw2= m.role!=='user'?'<span class="rw" onclick="RW('+i+')">✏</span>':'';
    ...
    else{var _segments=[];var _temp=m.content.replace(/\u3010([^\u3011]+)\u3011/g,'\nSEP_MARKER_START\u3010$1\u3011SEP_MARKER_END');var _parts=_temp.split('\n');_parts.forEach(function(_p){if(!_p.trim())return;var _ms=_p.match(/SEP_MARKER_START\u3010([^\u3011]+)\u3011SEP_MARKER_END/);if(_ms&&_s.length){var _mn2=_ms[1];var _found=false;for(var _j=0;_j<_s.length;_j++){if(_s[_j].name===_mn2){_segments.push({speaker:_mn2,avatar:_s[_j].avatar||a,text:_p.replace(_ms[0],'').trim()});_found=true;break}}if(!_found)_segments.push({speaker:n,avatar:a,text:_p.replace(_ms[0],'').trim()})}else{_segments.push({speaker:n,avatar:a,text:_p})}});if(!_segments.length)_segments.push({speaker:n,avatar:a,text:m.content});_segments.forEach(function(_seg){var _sa2=_seg.avatar||a;h+='<div class="msg-row a-row"><div class="aav">'+(_sa2.indexOf('http')===0||_sa2.indexOf('/uploads/')===0?'<img src="'+_sa2+'" style="width:28px;height:28px;border-radius:50%;object-fit:cover">':avt(_sa2,30))+'</div><div class="msg a"><div class="sn">'+E(_seg.speaker)+'</div><div>'+H(_seg.text)+'</div></div>'+rw+rw2+'</div>'})+='<div class="msg-row a-row"><div class="aav">'+(_sa.indexOf('http')===0||_sa.indexOf('/uploads/')===0?'<img src="'+_sa+'" style="width:28px;height:28px;border-radius:50%;object-fit:cover">':avt(_sa,30))+'</div><div class="msg a"><div class="sn">'+E(_sn)+'</div><div>'+H(_txt)+'</div></div>'+rw+rw2+'</div>'}}
